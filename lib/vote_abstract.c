...
vote_abstract_join_decend_tree(const vote_tree_t *t, size_t node_id,
  ...
  if(left_id < 0 || right_id < 0) {
    ...
    for(size_t i=0; i<nb_outputs; i++) {
      outputs[i].lower = vote_min(t->flt_ops, value[i], outputs[i].lower);
      outputs[i].upper = vote_max(t->flt_ops, value[i], outputs[i].upper);
    }
    return;
  }
  ...
  if(vote_le(t->flt_ops, inputs[dim].lower, threshold)) {
  ...
  if(vote_gt(t->flt_ops, inputs[dim].upper, threshold)) {
...
vote_abstract_join_tree(const vote_tree_t *t,
  ...
  vote_abstract_join_decend_tree(t, root_id, inputs, nb_inputs, outputs,
				 nb_outputs);
}
...
vote_abstract_join_trees(vote_tree_t *const*trees, size_t nb_trees,
  ...
  for(size_t i=0; i<nb_trees; i++) {    
    ...
    for(size_t dim=0; dim<nb_outputs; dim++) {
      outputs[dim].lower = vote_add(trees[i]->flt_ops, outputs[dim].lower,
				    tree_outputs[dim].lower);
      outputs[dim].upper = vote_add(trees[i]->flt_ops, outputs[dim].upper,
