...
vote_xgboost_one_vs_all(const vote_ensemble_t *e, size_t label) {
...
vote_xgboost_load(FILE* f) {
  ...
  for(int i=0; i<model_param.num_trees; i++) {
	...
	t->threshold[j] = vote_nextafter(t->flt_ops, node.value, -VOTE_INFINITY);
  ...
  if(e->nb_outputs > 1) {
    ...
    for(size_t i=0; i<e->nb_outputs; i++) {
      e->onevsall[i] = vote_xgboost_one_vs_all(e, i);
    }
  }
