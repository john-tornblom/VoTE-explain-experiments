...
class TestEnsembleBasics(SimpleVoTETestCase):
    ...
    def test_nb_trees(self):
        self.assertEqual(self.ensemble.nb_trees, 2)

    def test_nb_nodes(self):
        self.assertEqual(self.ensemble.nb_nodes, 14)

    def test_feature_usage(self):
        usage = self.ensemble.feature_usage()
...
class TestAbsRef(SimpleVoTETestCase):
    ...
    def test_exhaustive_absref(self):
        ...
        self.assertEqual(res, vote.PASS)
...
class TestCatBoost(unittest.TestCase):
    ...
    def test_binary_classification(self):
        ...
        self.assertEqual(e.absref(cb, dom), vote.PASS)
...
class TestXGBoost(unittest.TestCase):
    ...
    def test_binary_classification(self):
        ...
        cb = functools.partial(check_mapping, pred, np.float32, np.float32)
        dom = [(np.finfo(np.float32).min, np.finfo(np.float32).max)] * e.nb_inputs
    ...
    def test_multiclass_classification(self):
        ...
        X, Y = make_classification(n_classes=3, n_clusters_per_class=2,
                                   n_informative=3, random_state=12345)
        m = XGBClassifier(n_estimators=2, n_jobs=1, max_depth=3, random_state=12345)
        m.fit(X, Y)
        Y_pred = m.predict_proba(X)

        e = vote.Ensemble.from_xgboost(m)
        cb = functools.partial(check_mapping, m.predict_proba, np.float32, np.float32, epsilon=1e-3)
        dom = [(np.finfo(np.float32).min, np.finfo(np.float32).max)] * e.nb_inputs
...
class TestExplain(unittest.TestCase):
    ...
    def test_explain_minimal(self):
        xvec = [1] * self.ensemble.nb_inputs
        expl = self.ensemble.explain_minimal(xvec)
        self.assertEqual(len(expl), 3)

    def test_explain_minimum(self):
        xvec = [1] * self.ensemble.nb_inputs
        expl = self.ensemble.explain_minimum(xvec)
        self.assertEqual(len(expl), 3)
...
class TestExplain(unittest.TestCase):
    serialized_ensemble = '''{
      ...
      "trees": [{
         ...
         "right": [2, 4, 6, -1, -1, -1, -1], 
         "threshold": [0, 0, 0, 0, 0, 0, 0], 
         "value": [[0], [0], [0], [-0.581635236740112], [0.0593974180519581], 
                   [-0.189438208937645], [0.336416184902191]]
       }, {
         "feature": [3, 4, 5, -1, -1, -1, -1], 
         "left": [1, 3, 5, -1, -1, -1, -1], 
         "nb_inputs": 6, 
         ...
         "right": [2, 4, 6, -1, -1, -1, -1], 
         "threshold": [0, 0, 0, 0, 0, 0, 0], 
         "value": [[0], [0], [0], [-0.443240076303482], [0.234191164374352], 
                   [0.287753611803055], [-0.138462170958519]]
       }, {
         "feature": [0, 1, 5, -1, -1, -1, -1], 
         "left": [1, 3, 5, -1, -1, -1, -1], 
         "nb_inputs": 6, 
         ...
         "right": [2, 4, 6, -1, -1, -1, -1], 
         "threshold": [0, 0, 0, 0, 0, 0, 0], 
         "value": [[0], [0], [0], [-0.385746866464615], [0.0939076393842697], 
                   [0.108503617346287], [-0.16642951965332]]
      }]}'''
    ...
    def test_explain_minimal(self):
        xvec = [1] * self.ensemble.nb_inputs
        expl = self.ensemble.explain_minimal(xvec)
        self.assertTrue(self.ensemble.explain_is_valid(xvec, expl))
        self.assertEqual(expl, [1, 2, 4])
    ...
    def test_explain_forall(self):
        xvec = [1] * self.ensemble.nb_inputs
        expected = set([frozenset([1, 2, 4]), frozenset([0, 2, 4]),
                        frozenset([1, 2, 3]), frozenset([0, 2, 3])])
        found = set()

        def cb(expl):
            self.assertTrue(self.ensemble.explain_is_valid(xvec, expl))
            found.add(frozenset(expl))
            return True

        self.ensemble.explain_forall(xvec, cb)
