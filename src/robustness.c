...
analyze_sample(void* ctx) {
  ...
  if(vote_ensemble_absref(a->ensemble, bounds, is_correct, a) == VOTE_PASS) {
    for(size_t i=0; i<a->ensemble->nb_inputs; i++) {
...
analyze_robustness(robustness_analysis_t *a) {
  ...
  for(size_t row=0; row<nb_samples; row++) {
    ...
    workqueue_schedule(wq, analyze_sample, &analyses[row], a->sample_timeout);
  }
  ...
  real_t walltime = vote_timespec_diff(&start_clock, &stop_clock);
  size_t passed = 0;
  size_t timeouts = 0;
